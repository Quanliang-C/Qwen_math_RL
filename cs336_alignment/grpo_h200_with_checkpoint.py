...
from grpo_evaluate import evaluate_vllm_with_multiple
...
model_version = "v4_h200"


use_std_normalization = False

grpo_clip_range = 0.10

### 断点需要注意,id, 模型，优化器，lr scheduler等都要一致, 要设置start_train_step, 现在暂时是错误的
start_train_step = 1
Load_From_Checkpoint = False
checkpoint_dir = "grpo_checkpoint/reinforce_with_baseline/v4_h200_180"
...
def main():
    ...
    for train_step in range(start_train_step, n_grpo_steps):
        ...
        if train_step == 1:
            ...
            policy = AutoModelForCausalLM.from_pretrained(checkpoint_dir, local_files_only=True, torch_dtype="bfloat16", attn_implementation="flash_attention_2").to("cuda")
            optimizer = torch.optim.AdamW(
                policy.parameters(),
                lr=learning_rate,
                betas=(0.9, 0.95),
            ...
            llm = init_vllm(LOCAL_SNAPSHOT, "cuda", 99, gpu_memory_utilization=gpu_memory_utilization)
            load_policy_into_vllm_instance(policy, llm)
            wandb.watch(policy, log="gradients", log_freq=50)
        elif (train_step-1) % 10 == 0:
            pass
        else:
            llm = init_vllm(LOCAL_SNAPSHOT, "cuda", 99, gpu_memory_utilization=gpu_memory_utilization)
            load_policy_into_vllm_instance(policy, llm)
        ...
        for i in range(epoch_per_rollout_batch):
            ...
            for j in range(0, rollout_batch_size, micro_train_batch_size):
                ...
                old_log_probs_tensor, response_mask, new_lpg_probs_resp, masked_entropy = get_response_log_probs_tensor_and_response_mask(new_log_probs, old_log_probs_this_batch, token_entropy)
                mean_token_entropy += masked_entropy.item()
            ...
            print(f"total_loss on train step {train_step}: {total_loss}, epoch: {i}")
...
if __name__ == "__main__":
    ...
    load_policy_into_vllm_instance(policy, llm)
    evaluate_vllm_with_multiple(llm, f"grpo_{loss_type}_{model_version}_test_multiple_times", r1_zero_reward_fn, max_tokens=sampling_max_tokens)
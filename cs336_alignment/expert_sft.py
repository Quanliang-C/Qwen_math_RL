...
random.seed(99)
responses, answers, rendered_prompts= [], [], []


with open("outputs/expert_rollout_3.jsonl", "r", encoding="utf-8") as f:
    for line in f:
        line = line.strip()
        if not line:
            continue
        example = json.loads(line)
        if example["reward"] == 1.0:
            responses.append(example["response"])
            answers.append(example["ground_truth"])
            rendered_prompts.append(example["prompt"])
        else:
            continue
...
model = AutoModelForCausalLM.from_pretrained("models/expert_2",
                                            torch_dtype="bfloat16",
                                            attn_implementation="flash_attention_2").to("cuda")
...
def masked_normalize(
    ...
    if dim is None:
        mask_sum = mask_sum.sum().clamp(min=1e-8)
        s = masked_tensor.sum()/mask_sum
    else:
        mask_sum = mask_sum.sum(dim=dim).clamp(min=1e-8)
...
dataset = list(zip(rendered_prompts, responses))
...
for epoch in range(epochs):
    ...
    for i, batch in enumerate(dataloader):
        ...
        masked_entropy = (token_entropy * response_mask).sum() / response_mask.sum().clamp_min(1e-8)
        ...
        print("step", step, "token_entropy", masked_entropy.item())
...
model.save_pretrained("models/expert_3")

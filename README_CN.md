...
## 5.1 实验结果（GSM8K test split, 严格 0-shot）
...
| Qwen2.5-Math-7B (原始)       | ~6.1%           | -                   | 同一评测脚本下                   |
...
## 5.2 实验结果（GSM8K test split, 宽松 0-shot）

| 训练策略 / 模型                    | pass@1 Accuracy | 训练步骤                | 说明                               |
| ---------------------------- | --------------- | ------------------- | -------------------------------- |
| Qwen2.5-Math-1.5B (原始)       | ~30.5%           | -                   | 严格 0-shot 基线                     |
| REINFORCE+baseline (本系统)     | ~89%            | ~200 on-policy step | 无 KL/entropy 正则，仅靠组内 baseline 优势 |
| Dr GRPO / clipped GRPO (本系统) | ~84%            | ~200 step           | ratio clipping (clip≈0.30)，更稳但略低 |
| Qwen2.5-Math-7B (原始)       | ~64.7%           | -                   | 同一评测脚本下                   |
| Llama-3.1-8B-Instruct        | ~48.9%          | -                   | 同一评测脚本下                          |
| Qwen3-8B (thinking 模式)       | ~82.2%          | -                   | 同一评测脚本下                          |

说明：

* 所有对比模型都在“同一评测脚本、同一标准”下评测。
* 评测标准要求数值正确，不要求格式正确。

---


## 5.3 实验结果（MATH500, 严格 0-shot）

| 训练策略 / 模型                    | pass@1 Accuracy | 训练步骤                | 说明                               |
| ---------------------------- | --------------- | ------------------- | -------------------------------- |
| Qwen2.5-Math-1.5B (原始)       | ~4.6%           | -                   | 严格 0-shot 基线                     |
| REINFORCE+baseline (本系统)     | ~67.8%            | ~200 on-policy step | 无 KL/entropy 正则，仅靠组内 baseline 优势 |
| Qwen2.5-Math-7B (原始)       | ~2.8%           | -                   | 同一评测脚本下                   |
| Llama-3.1-8B-Instruct        | ~14.4%          | -                   | 同一评测脚本下                          |
| Qwen3-8B (thinking 模式)       | ~21.4%          | -                   | 同一评测脚本下                          |
...
## 5.4 实验结果（MATH500, 宽松 0-shot）

| 训练策略 / 模型                    | pass@1 Accuracy | 训练步骤                | 说明                               |
| ---------------------------- | --------------- | ------------------- | -------------------------------- |
| Qwen2.5-Math-1.5B (原始)       | ~23.2%           | -                   | 严格 0-shot 基线                     |
| REINFORCE+baseline (本系统)     | ~69.6%            | ~200 on-policy step | 无 KL/entropy 正则，仅靠组内 baseline 优势 |
| Qwen2.5-Math-7B (原始)       | ~57.2%           | -                   | 同一评测脚本下                   |
| Llama-3.1-8B-Instruct        | ~22.8%          | -                   | 同一评测脚本下                          |
| Qwen3-8B (thinking 模式)       | ~67.4%          | -                   | 同一评测脚本下                          |
